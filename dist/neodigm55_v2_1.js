...
class NeodigmSodaPop {
    constructor(_d, _aQ) {  //  Flux Capacitor
        ...
        this.fOnBeforeOpen = this.fOnAfterOpen = this.fOnClose = this.fOnBeforeUserExit = []
    ...
    close( _bFast ) {
        if(this.bIsInit && this.bIsOpen) {
            if(this.fOnClose.length) this.fOnClose[0]()//this.fOnClose.forEach( (f)=> f())
...
class NeodigmClaireAtomOn{
  ...
  draw(){
    ...
    this.complete = (this.size >= (  this.nMax * 1 ) )
if( this.complete ) console.log("COMPLETE ------")
...
<neodigm-snack class="l-snackbar" role="alert">
